...
<div class="container theme-showcase">
	...
	if($_SESSION["role"]["manager"] == 1) { 
		//topic_request query
		$stmt = $dbh->prepare("select topic_request.*, user.email, user_data.firstname, user_data.lastname from topic_request inner join user on user.id = topic_request.user_id inner join user_data on user.id = user_data.user_id");
		$stmt->execute();
	                ...
	                <tbody>
	                	...
	                	<tr id="<?php echo "topic_" . $i;?>">
							...
							<?php
		                		...
		                		if(isset($fetchTopicRequest[$i]["questionnaire_id"]))
		                		{
		                			$stmt = $dbh->prepare("select name from questionnaire where id = :qId");
		                			$stmt->bindParam(":qId", $fetchTopicRequest[$i]["questionnaire_id"]);
		                		} else if(isset($fetchTopicRequest[$i]["question_id"]))
		                		{
		                			$stmt = $dbh->prepare("select text from question where id = :qestionId");
		                			$stmt->bindParam(":qestionId", $fetchTopicRequest[$i]["question_id"]);
		                		}
